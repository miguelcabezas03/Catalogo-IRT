...
import { CheckCircle2, ChevronRight, Download, Expand, FileImage, FolderOpen, ImageIcon, LoaderCircle, LogOut, Search, ShieldCheck, Sparkles, Upload, UserRound, X } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const channel = client.channel('catalogo-compartido').on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'catalog_images' },
      () => {
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="max-h-[92vh] overflow-y-auto p-0 sm:max-w-4xl">
          {selected && <div className="grid min-h-[520px] md:grid-cols-[1.25fr_.75fr]">
            <div className="relative flex min-h-[310px] items-center justify-center overflow-hidden bg-[#0d2b49] p-10">
              ...
              {selected.imageUrl ? <img src={selected.imageUrl} alt={selected.name} className="relative max-h-[68vh] max-w-full object-contain" /> : <div className="relative flex flex-col items-center text-white/75"><FileImage className="size-20" /><p className="mt-4 text-lg font-medium">{selected.name}</p></div>}
            </div>
            <div className="flex flex-col p-6">
              <DialogHeader>
                <div className="mb-1 flex items-center gap-2"><Badge className="bg-[#e8f3f6] text-[#16526b]">{selected.country}</Badge>{selected.isNew && <Badge className="bg-amber-100 text-amber-800">Nueva</Badge>}</div>
                <DialogTitle className="text-xl text-[#102f4f]">{selected.name}</DialogTitle>
                <DialogDescription>{selected.folder}</DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter className="mt-7 bg-[#f5f8fa]"><Button variant="outline" onClick={() => setSelected(null)}>Cancelar</Button><Button onClick={() => void saveReview()} className="bg-[#d91471] text-white hover:bg-[#bd0e61]">Guardar observación</Button></DialogFooter>
            </div>
          </div>}
        </DialogContent>
      </Dialog>
